...
import { useProposalData, ProposalData } from '../../state/governance/hooks'
import { DateTime } from 'luxon'
import ReactMarkdown from 'react-markdown'
import VoteModal from '../../components/vote/VoteModal'
//import { TokenAmount, JSBI } from '@pangolindex/sdk'
//import { useTokenBalance } from '../../state/wallet/hooks'
//import { useActiveWeb3React } from '../../hooks'    // how does this one work?
...
//  const [support, setSupport] = useState<boolean>(true)
  const [support] = useState<boolean>(true)
  ...
  return (
    <PageWrapper gap="lg" justify="center">
      <VoteModal
        ...
        support={support}
      />
      {/* what is this ProposalInfo block? */}
      <ProposalInfo gap="lg" justify="start">
        <RowBetween style={{ width: '100%' }}>
          <ArrowWrapper to="/ido">
            <ArrowLeft size={20} /> {t('IDOPage.allIDOs')}
          </ArrowWrapper>
          {/* Show the current IDO status */}
          {proposalData && <ProposalStatus status={proposalData?.status ?? ''}>{proposalData?.status}</ProposalStatus>}
        </RowBetween>
        <AutoColumn gap="10px" style={{ width: '100%' }}>
          {/* Show IDO title */}
          <TYPE.largeHeader style={{ marginBottom: '.5rem' }}>{proposalData?.title}</TYPE.largeHeader>
          {/* Show IDO start and end date to participate */}
          <RowBetween>
            <TYPE.main>
