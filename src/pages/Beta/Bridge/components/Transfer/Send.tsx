...
import useAllowance from 'src/hooks/bridgeHooks/useAllowance'
import { useHandleTransfer } from 'src/hooks/bridgeHooks/useHandleTransfer'
...
function Send() {
  ...
  const toggleShouldApproveUnlimited = useCallback(() => setShouldApproveUnlimited(!shouldApproveUnlimited), [
      ...
      {approveButtonNeeded ? (
        <>
          ...
          <ButtonWithLoader
            ...
            error={errorMessage}
          >
            {'Approve ' + (shouldApproveUnlimited ? 'Unlimited' : sourceAmount) + ` Token${notOne ? 's' : ''}`}
          </ButtonWithLoader>
        </>
      ) : (
        <>
          ...
          <SendConfirmationDialog open={isConfirmOpen} onClick={handleConfirmClick} onClose={handleConfirmClose} />
        </>
      )}
