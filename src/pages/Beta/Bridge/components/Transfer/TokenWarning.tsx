...
import { AVAILABLE_MARKETS_URL, CHAINS_BY_ID, MULTI_CHAIN_TOKENS } from 'src/utils/bridgeUtils/consts'
  ...
  return (
    <div style={{ border: '1px solid #6DA8FF', padding: '15px', marginBottom: '10px' }}>
      <Text fontSize={15} fontWeight={300} lineHeight="20px" color="primaryText1">
        ...
        <Text fontSize={15} fontWeight={500} lineHeight="20px" color="primaryText1">
          Wormhole Wrapped Tokens
        </Text>{' '}
        and will need to be exchanged for native assets.
      </Text>
      <Text fontSize={15} fontWeight={300} lineHeight="20px" color="white">
        <a href={AVAILABLE_MARKETS_URL} target="_blank" rel="noopener noreferrer" style={{ color: '#6DA8FF' }}>
...
function MultichainWarning({ symbol, targetChain }: { symbol: string; targetChain: ChainId }) {
  return (
    <div style={{ border: '1px solid #6DA8FF', padding: '15px', margin: '15px' }}>
      ...
      <Text fontSize={17} fontWeight={300} lineHeight="20px" color="white">
        <a href={AVAILABLE_MARKETS_URL} target="_blank" rel="noopener noreferrer">
          Click here to see available markets for wrapped tokens.
        </a>
      </Text>
    </div>
...
function RewardsWarning() {
  return (
    <div style={{ border: '1px solid #6DA8FF', padding: '15px', margin: '15px' }}>
      Lido stETH rewards can only be received on Ethereum. Use the value accruing wrapper token wstETH instead.
    </div>
  )
}
...
export default function TokenWarning({
  ...
  if (!(originChain && targetChain && targetAsset && sourceChain && sourceAsset)) {
    return null
  }
  ...
  const multichainSymbol = MULTI_CHAIN_TOKENS[sourceChain]?.[searchableAddress] || undefined
  const isMultiChain = !!multichainSymbol
  const isRewardsToken =
    searchableAddress === '0xae7ab96520de3a18e5e111b5eaab095312d7fe84' && sourceChain === CHAIN_ID_ETH
  ...
  return (
    <>
      {showMultiChainWarning ? (
        <MultichainWarning symbol={multichainSymbol || 'tokens'} targetChain={targetChain} />
      ) : null}
