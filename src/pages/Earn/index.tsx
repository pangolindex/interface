...
export default function Earn({
  ...
  const [stakingInfoResult, setStakingInfoResult] = useState<StakingInfo>()
  const [stakingApr, setStakingApr] = useState<string>()

  useEffect(() => {
    if (stakingInfoResult) {
      fetch(`https://api.pangolin.exchange/pangolin/apr/${stakingInfoResult.stakingRewardAddress}`)
        .then(res => res.text())
        .then(res => setStakingApr(res))
    }
  }, [stakingInfoResult])
        ...
        <PoolSection>
          {stakingRewardsExist && stakingInfos?.length === 0 ? (
            ...
            stakingInfos
              ...
              .map(stakingInfo => {
                setStakingInfoResult(stakingInfo)
                return (
                  <PoolCard
                    apr={stakingApr || ''}
