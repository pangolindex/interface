...
export function useStakingInfo(version: number, pairToFilterBy?: Pair | null): DoubleSideStakingInfo[] {
  ...
  const usdPrice = chain.mainnet ? usdPriceTmp : undefined
...
export function useGetPairDataFromPair(pair: Pair) {
  ...
  const usdPriceCurrency0 = chain.mainnet ? usdPriceCurrency0Tmp : undefined
  const usdPriceCurrency1Tmp = useUSDCPrice(token1)
  const usdPriceCurrency1 = chain.mainnet ? usdPriceCurrency1Tmp : undefined
  ...
  const arr = useMemo(() => {
    ...
    return pairAddresses.reduce<any[]>((memo, pairAddress, index) => {
      ...
      if (
        ...
        let totalStakedInUsd = chain.mainnet
        ...
        if (JSBI.equal(totalSupplyAvailable, BIG_INT_ZERO)) {
          ...
          totalStakedInUsd = chain.mainnet
...
export function useGetPoolDollerWorth(pair: Pair | null) {
  ...
  const currency0Price = chain.mainnet ? currency0PriceTmp : undefined

  const userPglTmp = useTokenBalance(account ?? undefined, pair?.liquidityToken)
  const userPgl = chain.mainnet ? userPglTmp : undefined
  ...
  const liquidityInUSD = chain.mainnet
